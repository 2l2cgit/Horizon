...
spawn(function() 
    pcall(function() 
        if getconnections then 
            ...
            for i,v in pairs(getconnections(game:GetService("ScriptContext").Error)) do
                v:Enable() -- enable connections since Horizon is not injected, thus we dont care.
            end
...
local SwearDetection = loadstring(requesturl("Horizon/lib/swear-detection.lua"))()
...
do 
    ...
    spammer = GuiLibrary["Objects"]["MiscellaneousWindow"]["API"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
                if spammerfile["Value"] ~= "" and isfile("Horizon/"..spammerfile["Value"]) then
                    messagetable = readfile(("Horizon/"..spammerfile["Value"]))
                ...
                elseif spammer["Enabled"] then
                    ...
                    fprint("Horizon/"..value.." is not a valid file, please make sure it is in your workspace folder and you include the file extension")
                end
            end
        end
    })
    spammerfile = spammer.CreateTextbox({
        ["Name"] = "file.txt",
        ["Function"] = function(value)
            if value ~= "" and isfile("Horizon/"..value) then 
                messagetable = readfile(("Horizon/"..value)):split("\n")
                if spammer["Enabled"] then 
                    spammer.Toggle()
                    spammer.Toggle()
                end
...
do 
    ...
    Horizon.AddCommand("search", {Function = function(args) 
    ...
    GuiLibrary.SaveConfig = function(name, isAutosave) 
        local path = ("Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId).."/"..name..".search.json")
...
do
    local function addtofile(plr, reason, result) 
        if not betterisfile("Horizon/reported.txt") then 
            writefile("Horizon/reported.txt", "-- HorizonClient.xyz AutoReport logs.\n-- by engo#0320\n\n--LOG BEGIN --\n")
        end

        appendfile("Horizon/reported.txt", "reported "..plr.Name.." ("..tostring(plr.UserId)..") for "..reason.." for saying '"..result.."'\n")
    end
    ...
    local function addtoqueue(msg, plr)
        ...
        if result and reason and (not isqueued(tab)) then 
            ...
            if not suc and shared.HorizonDeveloper then 
                warn(ret)
            end
        end
    end
