...
local GuiLibrary = Horizon.GuiLibrary
...
local function canBeTargeted(plr, doTeamCheck) 
    return Horizon.canBeTargeted(plr)
end
...
do
    ...
    local suffix = " | Horizonclient.xyz"
    local sensitives = {
        "hack",
        "exploit",
        "script",
                    ...
                    if v ~= lplr then
                        connections[#connections+1] = v.Chatted:connect(function(msg) 
                            if msg:find("vxpe") then
                                game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("vxpe < Horizonclient.xyz", "All")
                            end
                ...
                BindToStepped("Nametags", function() 
                    for i,v in next, PLAYERS:GetPlayers() do 
                        if lplr~=v and isAlive(v) then
                            ...
                            local blue = "#2a96fa"
                            local red = "#ed4d4d"
                            local text = '<font color="'..(Horizon.isFriend(v) and blue or red)..'">'..v.DisplayName..'</font>'..(tagshealth.Enabled and ' <font color="#'..(convertHealthToColor(v.Character.Humanoid.Health, v.Character.Humanoid.MaxHealth):ToHex())..'">'..tostring(math.round(v.Character.Humanoid.Health))..'</font>' or '')
...
local function PrepareSessionInfo() 
    ...
    if betterisfile("Horizon/configs/SessionInfo.json") then 
        local suc, value = pcall(function() 
            return HTTPSERVICE:JSONDecode(readfile("Horizon/configs/SessionInfo.json"))
        end)
        if suc then 
            posTable = value
        end
    ...
    LagbacksValue.Text = shared.HorizonSavedSessionInfo and shared.HorizonSavedSessionInfo.lagbacks or "0"
    ...
    KillsValue.Text = shared.HorizonSavedSessionInfo and shared.HorizonSavedSessionInfo.kills or "0"
    ...
    WinsValue.Text = shared.HorizonSavedSessionInfo and shared.HorizonSavedSessionInfo.wins or "0"
...
local ontp = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
		...
		local stringtp = "shared.HorizonSavedSessionInfo = {startTime ="..tostring(shared.HorizonStartTime)..", kills = "..api.kills.Text..", wins = "..api.wins.Text..", lagbacks = "..api.lagbacks.Text.."}"
...
spawn(function()
    repeat task.wait(0.5) 
        ...
        local t = math.round(WORKSPACE:GetServerTimeNow()) - math.round((shared.HorizonSavedSessionInfo and tonumber(shared.HorizonSavedSessionInfo.startTime)) or shared.HorizonStartTime)
...
GuiLibrary.Signals.onDestroy:connect(function()
    local api = SessionInfoAPI
    shared.HorizonSavedSessionInfo = {startTime = tostring(shared.HorizonStartTime), kills = api.kills.Text, wins = api.wins.Text, lagbacks = api.lagbacks.Text}
    ...
    if suc then 
        if betterisfile("Horizon/configs/SessionInfo.json") then 
            delfile("Horizon/configs/SessionInfo.json")
        end
        writefile("Horizon/configs/SessionInfo.json", value)
    else
        error(value)
    end
...
local clients = {
	ChatStrings1 = {
		["KVOP25KYFPPP4"] = "vape",
		["IO12GP56P4LGR"] = "Horizon"
	},
	ChatStrings2 = {
		["vape"] = "KVOP25KYFPPP4",
		["Horizon"] = "IO12GP56P4LGR"
	},
	ClientUsers = {}
}
...
local connection1 = game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:connect(function(tab, channel)
	...
	if plr and bedwars["CheckPlayerType"](lplr) ~= "DEFAULT" and tab.MessageType == "Whisper" and client ~= nil and alreadysaidlist[plr.Name] == nil then
		...
		GuiLibrary.CreateToast("Horizon", plr.Name.." is using "..client.."!", 60)
...
local function fun(plr) 
    if lplr ~= plr and bedwars["CheckPlayerType"](lplr) == "DEFAULT" and bedwars["CheckPlayerType"](plr) ~= "DEFAULT" then
        spawn(function()
            ...
            task.wait(4)
            game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..clients.ChatStrings2.Horizon, "All")
            spawn(function()
                local connection
                for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
                    if newbubble:IsA("TextLabel") and newbubble.Text:find(clients.ChatStrings2.Horizon) then
                        pcall(function()
                            newbubble.Parent.Parent.Visible = false
