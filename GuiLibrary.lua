-- // New gui library for Horizon roblox.
...
makefolder("Horizon")
makefolder("Horizon/logs")
makefolder("Horizon/assets")
makefolder("Horizon/configs")
makefolder("Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId))
makefolder("Horizon/configs/!SelectedConfigs/")
...
--[[
if betterisfile("Horizon/logs/latestmove.log") then 
    local data = readfile("Horizon/logs/latestmove.log")
    delfile("Horizon/logs/latestmove.log")
    local date = data:split("\n")[1]
    writefile(("Horizon/logs/"..date:gsub(" ", "_"):gsub("[^%w%s_:-]+", ""):gsub(":", "-")..".log"), data)
end
if betterisfile("Horizon/latest.log") then 
    local data = readfile("Horizon/latest.log")
    delfile("Horizon/latest.log")
    writefile(("Horizon/logs/latestmove.log"), data)
end
local function log(sys, mes) 
    ...
    if not betterisfile("Horizon/latest.log") then 
        writefile("Horizon/latest.log", os.date("%c").."\n"..toPush)
    else
        appendfile("Horizon/latest.log", toPush)
    end
end
...
local function playclicksound() 
    if GuiLibrary["ClickSounds"] then
        playsound(getasset("Horizon/assets/click.mp3"))
    end
end
...
GuiLibrary["PrepareWatermark"] = function()
    ...
    function api.draw() 
        ...
        Watermark.Text = "Horizon"..(isfolder("Horizon/plus") and "+" or "").." v"..tostring(shared._HorizonVERSION).." | "..tostring(shared._HorizonMOTD)
...
GuiLibrary["CreateNotification"] = function(content)
    if GuiLibrary["AllowNotifications"] then
        chatchildaddedconnection = chatchildaddedconnection or PLAYERS.LocalPlayer.PlayerGui.Chat.Frame.ChatChannelParentFrame["Frame_MessageLogDisplay"].Scroller.ChildAdded:Connect(function(child) 
            if child:FindFirstChildOfClass("TextLabel").Text:find("[Horizon]") then 
                child:FindFirstChildOfClass("TextLabel").RichText = true
            end
        end)
        STARTERGUI:SetCore("ChatMakeSystemMessage", {["Text"] = "\n<font color='rgb(255, 85, 85)'>[Horizon]</font> <font color='rgb(200,200,200)'>"..tostring(content).."</font>"})
    end
end
GuiLibrary["Debug"] = function(content) 
    if not shared.HorizonDebug then return end
    print("[Horizon] [DEBUG] "..content)
end
GuiLibrary["SaveConfig"] = function(name, isAutosave) 
    if shared.Horizon==nil or shared.Horizon.Destructing then return end
    local name = (name == nil or name == "") and "default" or name
    GuiLibrary["Debug"]("save Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId).."/"..name..".json")
    local config = {}
    ...
    local path = "Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId).."/"..name..".json"
    makefolder("Horizon/configs")
    makefolder("Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId))
    ...
    repeat task.wait() until betterisfile((path))
    if betterisfile("Horizon/configs/GUIconfig.json") then 
        delfile("Horizon/configs/GUIconfig.json")
    end
    writefile("Horizon/configs/GUIconfig.json", HTTPSERVICE:JSONEncode(guiconfig))
    repeat task.wait() until betterisfile("Horizon/configs/GUIconfig.json")
end
GuiLibrary["LoadOnlyGuiConfig"] = function() 
    if betterisfile("Horizon/configs/GUIconfig.json") then 
        local success, config = pcall(function() 
            local x = readfile("Horizon/configs/GUIconfig.json")
        ...
        if success then 
            ...
            warn("[Horizon] Failed to load GUIconfig.json config file")
        end
    else
        for i,v in next, GuiLibrary.Objects do 
...
GuiLibrary["LoadConfig"] = function(name) 
    ...
    writefile("Horizon/configs/!SelectedConfigs/"..tostring(shared.HorizonPlaceId or game.PlaceId)..".txt", name) 
    GuiLibrary["Debug"]("Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId).."/"..name..".json")
    if betterisfile("Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId).."/"..name..".json") then 
        print("[Horizon] Loading configuration "..name)
        log("LoadConfig", "Loading "..name)
        local success, config = pcall(function() 
            local x = readfile("Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId).."/"..name..".json")
            return HTTPSERVICE:JSONDecode(x)
        end)
        if success then 
            print("[Horizon] Success loading configuration "..name)
            ...
            warn("[Horizon] Failed to load "..tostring(shared.HorizonPlaceId or game.PlaceId)..".json config file\nplease report this in the discord!\n("..config..")")
        end
    end
...
GuiLibrary["CreateWindow"] = function(argstable)
    ...
    Expand.ZIndex = 1
    Expand.Image = getasset("Horizon/assets/arrow.png") --"rbxassetid://8904422926"
    ...
    windowapi["CreateOptionsButton"] = function(argstable) 
        ...
        Gear.Image = getasset("Horizon/assets/gear.png") --"rbxassetid://8905804106"
        ...
        buttonapi["CreateSelector"] = function(argstable) 
            ...
            selectorapi["SelectNext"] = function() 
                ...
                if newindex then 
                    ...
                    warn("[Horizon] NewIndex in selector ("..argstable.Name..") in function `SelectNext` was not found!")
                end
                playclicksound()
            end
            ...
            selectorapi["SelectPrevious"] = function() 
                ...
                if newindex then 
                    ...
                    warn("[Horizon] NewIndex in selector ("..argstable.Name..") in function `SelectPrevious` was not found!")
                end
                playclicksound()
            end
