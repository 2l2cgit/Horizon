...
shared.HorizonStartTime = game:GetService("Workspace"):GetServerTimeNow()
local startTime = shared.HorizonStartTime
shared.Horizon = {}
local Horizon = shared.Horizon
...
--shared.Horizon.entity = loadstring(game:HttpGet("https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/main/Libraries/entityHandler.lua"))()

if game:GetService("CoreGui"):FindFirstChild("RobloxVRGui") then 
    game:GetService("CoreGui"):FindFirstChild("RobloxVRGui"):Destroy()
end

local GuiLibrary = loadstring(requesturl("Horizon/GuiLibrary.lua"))()

shared.Horizon.GuiLibrary = GuiLibrary
...
local function getasset(path)
	if not betterisfile(path) then
        ...
        print("[Horizon] downloaded "..path.." asset successfully!")
	end
	return getcustomasset(path) 
end

local function getscript(id) 
    local id = id or shared.HorizonPlaceId or game.PlaceId
    id = tostring(id)
    local req = requesturl("Horizon/scripts/"..id..".lua")
    if type(req) == "string" then
        return loadstring(req)()
    else
        --fwarn("[Horizon] invalid script (error "..tostring(req)..")") -- game is not supported
    end
end

local function getplusscript(id) -- Horizon plus moment
    local id = id or shared.HorizonPlaceId or game.PlaceId
    id = tostring(id)
    if not betterisfile("Horizon/plus/"..id..".fp") then return end
    local req = readfile("Horizon/plus/"..id..".fp")
    if type(req) == "string" then
        return loadstring(req)()
    else
        --fwarn("[Horizon] invalid script (error "..tostring(req)..")") -- game is not supported
...
local friendstab = {pcall(function() HTTPSERVICE:JSONDecode(readfile("Horizon/Friends.json")) end)}
Horizon.Friends = friendstab[1] and friendstab[2] or {}

Horizon.isFriend = function(plr) 
    return Horizon.Friends[plr.Name:lower()] and true or false
end

Horizon.addFriend = function(plrname) 
    if not Horizon.Friends[plrname:lower()] then
        Horizon.Friends[plrname:lower()] = true
        GuiLibrary.CreateNotification("Successfully added "..plrname.." to your friends list!")
    end
end

Horizon.delFriend = function(plrname) 
    if Horizon.Friends[plrname:lower()] then
        Horizon.Friends[plrname:lower()] = nil
        GuiLibrary.CreateNotification("Successfully removed "..plrname.." from your friends list!")
    end
end
Horizon.removeFriend = Horizon.delFriend

Horizon.toggleFriend = function(plrname) 
    if Horizon.Friends[plrname:lower()] then 
        Horizon.removeFriend(plrname)
    else
        Horizon.addFriend(plrname)
    end
end

Horizon.canBeTargeted = function(plr) 
    if Horizon.isFriend(plr) then return false end
...
local configBox; configBox = configButton.CreateTextbox({
    ...
    ["Function"] = function(value)
        spawn(function()
            ...
            if betterisfile("Horizon/configs/"..tostring(shared.HorizonPlaceId or game.PlaceId).."/"..value..".json") then
...
if betterisfile("Horizon/configs/!SelectedConfigs/"..tostring(shared.HorizonPlaceId or game.PlaceId)..".txt") then 
    GuiLibrary.CurrentConfig = readfile("Horizon/configs/!SelectedConfigs/"..tostring(shared.HorizonPlaceId or game.PlaceId)..".txt") 
    configBox.Set(GuiLibrary.CurrentConfig, true)
    print("[Horizon] Detected config "..GuiLibrary.CurrentConfig.." used last time!")
end
        ...
        if callback then 
            spawn(function() 
                ...
                if shared.HorizonDeveloper then 
                    ...
                    loadstring(game:HttpGet('https://raw.githubusercontent.com/2l2cgit/Horizon/main/loadstring.lua', true))()
                end
            end)
        end
    end
...
commands.friend = {
    Function = function(args) 
        ...
        if mode == "list" then 
            local count = 0
            for i,v in next, Horizon.Friends do 
                count = count + 1
            end
            return GuiLibrary.CreateNotification("Friends ("..tostring(count).."): "..keyconcat(Horizon.Friends, ", "))
        end

        if not plrname then return end

        if mode == "add" then
            Horizon.addFriend(plrname:lower())
        elseif mode == "del" or mode == "remove" or mode == "delete" then
            if Horizon.Friends[plrname:lower()] then 
                Horizon.delFriend(plrname:lower())
            else
                GuiLibrary.CreateNotification(plrname.." is not in your friends list!")
            end
        end

        writefile("Horizon/Friends.json", HTTPSERVICE:JSONEncode(Horizon.Friends))
    end,
...
shared.Horizon.AddCommand = function(name, func) 
    commands[name] = func
end
...
local ontp = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
        ...
        if shared.HorizonDeveloper then 
            ...
            loadstring(game:HttpGet("https://raw.githubusercontent.com/2l2cgit/Horizon/main/Initiate.lua", true))() 
        end
        ]]
...
if table.find(bedwarsidtable, game.PlaceId) then 
    shared.HorizonPlaceId = 6872274481
end
...
if table.find(minerscaveidtable, game.PlaceId) then 
    shared.HorizonPlaceId = 6604417568
end
...
if success then 
    print("[Horizon] Successfully retrieved Universal script!")
else
...
if success2 then 
    print("[Horizon] Successfully retrieved Game script!")
else
...
if success3 then 
    print("[Horizon] Successfully retrieved HorizonPlus Game script!")
else
    fwarn("Unsuccessful attempt at retrieving HorizonPlus Game script!\n (".._error3..")")
    GuiLibrary.Signals.onDestroy:Fire()
    return
end
GuiLibrary["LoadConfig"](GuiLibrary["CurrentConfig"])

-- Horizon command system
...
if not suc then warn("[Horizon] Chat hook failed, aborting command system. \n(Error: "..res..")") end
...
GuiLibrary.Signals.onDestroy:connect(function()
    ...
    shared.Horizon.Destructing = true
    writefile("Horizon/configs/!SelectedConfigs/"..tostring(shared.HorizonPlaceId)..".txt", GuiLibrary.CurrentConfig) 
...
spawn(function()
    if GuiLibrary["AllowNotifications"] then
        ...
        textlabel.Text = [[<stroke thickness="2">Please join the Horizon discord server for updates and to leave feedback. discord.gg/bdjT5UmmDJ</stroke>]]
...
spawn(function()
    repeat
        if not shared.Horizon or shared.Horizon.Destructing then 
            break
        end
        for i = 1, 100 do 
            task.wait(0.02)
